...
export default async function BracketPage({
  ...
  // Compute y-position of every bout: first round evenly spaced, later
  // rounds positioned at the midpoint of their two children (via
  // next_bout_id), so the tree lines up visually like a real bracket.
  ...
  // Round theme, shown per-column when set on any bout in that round.
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-8">
          ...
          >
            {connectors.map((c, i) => (
              <path
                ...
                d={c.d}
                fill="none"
                stroke={c.live ? "var(--gold)" : "var(--border)"}
                strokeWidth={2}
              />
            ))}
          </svg>
      ...
      {/* Rules of engagement, per round with a theme set */}
      {themeByRound.size > 0 && (
